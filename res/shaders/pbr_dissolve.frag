...
void main() {
  ...
  // add a 1d texture to use as a curve
  //  add a variation of this shader that starts from the center
  float speed = 0.2;
  ...
  float edgeSize = 0.2;
  if (noiseValue < progress + progress * edgeSize) {
    fragColor = vec4(2.0, 0.5, 10.0, 1.0);
    return;
  }
